...
import { KantinPenitip } from "@/lib/hooks/useKantin";
...
export default function InputTerjualModal({ penitip, onClose, onSubmit }: InputTerjualModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setSaving(true);
    await onSubmit(Object.entries(values).map(([snackId, qty]) => ({ snackId, qty })));
    setSaving(false);
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-3">
          {penitip.snacks.map((snack) => {
            ...
            const totalModal  = qty * snack.harga_beli;
            const omsetKantin = totalLaku - totalModal;
              ...
              <div key={snack.id} className="bg-gray-50 rounded-xl p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-xs text-gray-400">
                    Titip: {snack.qty_titip} · Beli: {formatRupiah(snack.harga_beli)} · Jual: {formatRupiah(snack.harga_jual)}
                  </span>
                </div>
                ...
                {qty > 0 && (
                  <div className="mt-3 grid grid-cols-3 gap-2 text-xs">
                    <div className="bg-blue-50 rounded-lg px-3 py-2 text-center">
                      <p className="text-gray-500">Balik Modal Penitip</p>
                      <p className="font-bold text-blue-700">{formatRupiah(totalModal)}</p>
                    </div>
                    ...
                    <div className="bg-amber-50 rounded-lg px-3 py-2 text-center">
                      <p className="text-gray-500">Omset Kantin</p>
                      <p className="font-bold text-amber-600">{formatRupiah(omsetKantin)}</p>
                    </div>
                  </div>
                )}
              </div>
