...
export default function RekapSesiModal({ sesi, penitips, onClose }: RekapSesiModalProps) {
  const totalOmsetKantin = penitips.reduce((s, p) => s + totalPenitip(p).omset_kantin, 0);
  const totalModalPenitip = penitips.reduce((s, p) => s + totalPenitip(p).total_modal, 0);
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-blue-50 rounded-xl p-3 text-center">
            <p className="text-xs text-gray-500">Total Laku</p>
            <p className="font-bold text-blue-700 text-sm mt-1">{formatRupiah(totalLaku)}</p>
          </div>
          <div className="bg-green-50 rounded-xl p-3 text-center">
            <p className="text-xs text-gray-500">Omset Kantin</p>
            <p className="font-bold text-green-700 text-sm mt-1">{formatRupiah(totalOmsetKantin)}</p>
          </div>
          <div className="bg-gray-50 rounded-xl p-3 text-center">
            <p className="text-xs text-gray-500">Balik Modal Penitip</p>
            <p className="font-bold text-gray-700 text-sm mt-1">{formatRupiah(totalModalPenitip)}</p>
          </div>
        </div>
                ...
                <div className="flex items-center justify-between px-4 py-2.5 bg-gray-50">
                  ...
                  <div className="flex gap-3 text-xs items-center">
                    <span className="text-gray-700 font-semibold">
                      Balik modal: {formatRupiah(tot.total_modal)}
                    </span>
                    <span className="text-green-700 font-semibold">
                      Omset kantin: {formatRupiah(tot.omset_kantin)}
                    </span>
                    {totalSisa > 0 && (
                    ...
                    <div key={snack.id} className="flex items-center px-4 py-2.5 text-sm">
                      <div className="flex-1">
                        ...
                        <span className="text-gray-400 ml-2 text-xs">
                          {snack.qty_terjual}/{snack.qty_titip} terjual ·
                          beli {formatRupiah(snack.harga_beli)} · jual {formatRupiah(snack.harga_jual)}
                        </span>
                      </div>
                      <div className="flex gap-4 text-xs items-center">
                        <span className="text-gray-500">
                          modal: {formatRupiah(snack.total_modal)}
                        </span>
                        <span className="text-green-600 font-semibold">
                          +{formatRupiah(snack.omset_kantin)}
                        </span>
                      </div>
                    </div>
                  ))}
        ...
        <div className="border-t-2 border-dashed border-gray-200 pt-4 space-y-1.5">
          <div className="flex justify-between text-sm">
            <span className="text-gray-500">Total uang dikembalikan ke penitip (balik modal)</span>
            <span className="font-bold text-gray-700">{formatRupiah(totalModalPenitip)}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-gray-500">Omset kantin (keuntungan jual - beli)</span>
            <span className="font-bold text-green-700">{formatRupiah(totalOmsetKantin)}</span>
          </div>
        </div>
